  ...
  return (
    <Box bg={duplicate ? 'orange.50' : 'green.50'} borderWidth="1px" borderColor={duplicate ? 'orange.200' : 'green.200'} borderRadius="2xl" p={{ base: 4, md: 6 }}>
      ...
      <Grid templateColumns={{ base: '1fr', md: 'repeat(3, 1fr)' }} gap={3} mt={5}>
        <Box bg="white" borderRadius="xl" p={4}><Text fontSize="xs" color="gray.500">Ingreso</Text><Text fontWeight="800">{person.entryAt ? peruTime(person.entryAt) : 'Pendiente'}</Text></Box>
        <Box bg="white" borderRadius="xl" p={4}><Text fontSize="xs" color="gray.500">Salida</Text><Text fontWeight="800">{person.exitAt ? peruTime(person.exitAt) : 'Pendiente'}</Text></Box>
...
export default function AttendanceScanner() {
  ...
  const previewCode = (value, detectedMode = mode) => {
    ...
    setCode(clean)
    setScanning(false)
    const person = people.find((p) => normalizeCode(p.documentNumber) === clean || normalizeCode(p.employeeCode) === clean)
  ...
  const confirmMark = async () => {
    ...
    try {
      ...
      await api.post(`/attendance/${selected.sheet.id}/scan-code`, { code, markingType, deviceInfo })
      ...
      await openSheet(selected.sheet.id)
      await load()
    } catch (e) {
        ...
        <Box bg="white" borderWidth="1px" borderRadius="xl" p={5} position={{ xl: 'sticky' }} top={{ xl: 4 }}>
          <Grid gap={4}>
            ...
            <FormControl>
              ...
              <Select value={markingType} onChange={(e) => { setMarkingType(e.target.value); setCandidate(null); setMessage('') }}>
            ...
            <FormControl>
              ...
              <Select value={mode} onChange={(e) => { setMode(e.target.value); setCandidate(null); setMessage(''); setScanning(false); setCode('') }}>
        ...
        <Grid gap={5}>
          <Box bg="white" borderWidth="1px" borderRadius="xl" p={{ base: 4, md: 6 }}>
            <Flex justify="space-between" gap={4} wrap="wrap" mb={5}>
              ...
              <Button variant="outline" onClick={() => { setCandidate(null); setMessage(''); setCode(''); setScanning(false); setTimeout(() => inputRef.current?.focus(), 80) }}>Limpiar</Button>
            </Flex>

            {mode === 'laser' && <FormControl>
              <FormLabel>DNI / Fotocheck</FormLabel>
              <Input ref={inputRef} size="lg" bg="gray.50" value={code} onChange={(e) => setCode(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') previewCode(e.currentTarget.value, 'laser') }} placeholder="Escanear o escribir código" autoComplete="off" />
            ...
            {mode === 'mobile' && <Box>
              {scanning ? <QrCameraScanner active={scanning} onScan={(value) => previewCode(value, 'mobile')} /> : <Button h="64px" w="100%" colorScheme="teal" fontSize="lg" onClick={() => { setCandidate(null); setMessage(''); setScanning(true) }}>Abrir cámara</Button>}
          ...
          {candidate && <Flex justify="flex-end" gap={3} wrap="wrap">
            <Button variant="outline" onClick={() => { setCandidate(null); setCode(''); setMessage(''); setTimeout(() => inputRef.current?.focus(), 80) }}>Cancelar</Button>
            ...
            <Table size="sm">
              ...
              <Tbody>
                {people.map((p) => <Tr key={p.collaboratorId} bg={candidate?.collaboratorId === p.collaboratorId ? 'teal.50' : undefined}>
                  ...
                  <Td>{p.entryAt ? peruTime(p.entryAt) : 'Pendiente'}</Td>
                  <Td>{p.exitAt ? peruTime(p.exitAt) : 'Pendiente'}</Td>
